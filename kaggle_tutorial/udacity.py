...
def custom_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        ...
        #
        passenger_id = passenger['PassengerId']
        if (passenger['Sex'] == 'female') or (passenger['Pclass'] == 1 and passenger['Age'] <= 15) \
                or (passenger['Pclass'] == 2 and passenger['Age'] <= 15):
            predictions[passenger_id] = 1
        else:
            predictions[passenger_id] = 0
...
## 80.2% accuracy on Udacity,


pred = custom_heuristic('data/test.csv')
